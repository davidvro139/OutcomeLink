...
<<<<<<< HEAD
=======
      // Provenance: the delivered file identifies itself, not just its data.
      const workbook = new ExcelJS.Workbook();
      await workbook.xlsx.load(download.body as Parameters<typeof workbook.xlsx.load>[0]);
      const info = workbook.getWorksheet("Report Info");
      expect(info).toBeDefined();
      const labels: string[] = [];
      info!.eachRow((row) => labels.push(String(row.getCell(1).value)));
      expect(labels).toEqual(expect.arrayContaining(["Name", "Generated at", "Generated by", "Rows"]));
